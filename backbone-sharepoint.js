...
(function (Backbone, _, $) {
  ...
  Backbone.SP.Item = Backbone.Model.extend({
    ...
    _updateChangeSet: function (model) {
    ...
    sync: function (method, model, options) {
      ...
      // transfer special url parameters like select and orderby to the params.data hash
      if (method == 'read') {
        params.data = params.data || {};
        _(['filter', 'select', 'orderby', 'top', 'skip', 'expand', 'inlinecount']).each(function (keyword) {
          if (options[keyword]) {
  ...
  Backbone.SP.List = Backbone.Collection.extend({
    ...
    parse: function (response) {
      ...
      if (response.d.__count) {
        this._count = parseInt(response.d.__count)
      } else { 
        delete this._count
      }
